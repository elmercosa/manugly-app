...
export default function Roles() {
  const BusinessContext = useBusiness();
  const columns = employeeRolesService.columns;
  const queryKey = "employees-roles";
  const queryFn = entityService("roles").getAll;
  ...
  return (
    <EntityTable
      ...
      entityName="rol de empleado"
      entityNamePlural="roles de empleado"
      endpoint="roles"
    />
  );
}
